...
class Selection_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        bio_id = self.bio2tensor(bio)#bio_id:tensor([2, 2, 0, 1, 1, 1, 2
        selection_id = self.selection2tensor(text, selection)#100*56*100    56是predicate55+N
    ...
    def pad_bert(self, text: List[str], bio: List[str], selection: List[Dict[str, int]]) -> Tuple[List[str], List[str], Dict[str, int]]:
        ...
        if len(text) > self.hyper.max_text_len:
            text = text[:self.hyper.max_text_len-1] + ['[SEP]']
            bio = bio[:self.hyper.max_text_len-1] + ['O']
    ...
    def selection2tensor(self, text, selection):
        ...
        NA = self.relation_vocab['N']#NA：0
        #这里有问题  初始化的时候 应该初始化为1
        #result[:, NA, :] = 1
        #seqence范围内
        seqence_len = text.index('[SEP]')
        result[1:seqence_len, NA, 1:seqence_len] = 1
...
class Selection_Test_Dataset(Dataset):
    def __init__(self, hyper, dataset):
        self.hyper = hyper
        self.data_root = hyper.data_root
    ...
    def __getitem__(self, index):
        ...
        if self.hyper.cell_name == 'bert':
            text = ['[CLS]'] + list(text) + ['[SEP]']
            if len(text) > self.hyper.max_text_len:
                text = text[:self.hyper.max_text_len]
            text = text + ['[PAD]'] * (self.hyper.max_text_len - len(text))
...
class Batch_test_reader(object):
    def __init__(self, data):
        transposed_data = list(zip(*data))
        # tokens_id,bio_id,text
...
def collate_test_fn(batch):
    return Batch_test_reader(batch)


Selection_loader = partial(DataLoader, collate_fn=collate_fn, pin_memory=True)

Selection_test_loader = partial(DataLoader, collate_fn=collate_test_fn, pin_memory=True)