...
from lib.dataloaders import Selection_Dataset, Selection_loader, Selection_Test_Dataset, Selection_test_loader
...
class Runner(object):
    def __init__(self, exp_name: str):
        ...
        self.model_dir = 'saved_lstm_model'
    ...
    def evaluation(self):
        ...
        self.model.eval()
        fwjson = open("result_dev1.json", 'w')

        pbar = tqdm(enumerate(BackgroundGenerator(loader)), total=len(loader))

        with torch.no_grad():
            for batch_ndx, sample in pbar:
                output = self.model(sample, is_train=False, is_test = False)
                self.triplet_metrics(output['selection_triplets'], output['spo_gold'])
                text_spo_dict = {}
                text_spo_dict['text'] = dev_set.text_list[batch_ndx]
                text_spo_dict['spo_list'] = []
                if len(output['selection_triplets'][0]) != 0:
                    text_spo_dict['spo_list'] = output['selection_triplets'][0]
                else:
                    text_spo_dict['spo_list'] = []

                fwjson.write(json.dumps(text_spo_dict, ensure_ascii=False) + '\n')
    ...
    def train(self):
        ...
        for epoch in range(self.hyper.epoch_num):
            ...
            for batch_idx, sample in pbar:
                ...
                output = self.model(sample, is_train=True, is_test=False)
                loss = output['loss']
                loss.backward()
                self.optimizer.step()
    ...
    def test(self):
        test_set = Selection_Test_Dataset(self.hyper, self.hyper.test)
        loader = Selection_test_loader(test_set, batch_size=self.hyper.test_batch, pin_memory=True)
        #self.triplet_metrics.reset()
        self.model.eval()
        fwjson = open("result_test1.json", 'w')

        pbar = tqdm(enumerate(BackgroundGenerator(loader)), total=len(loader))

        with torch.no_grad():
            for batch_ndx, sample in pbar:
                output = self.model(sample, is_train=False, is_test=True)
                text_spo_dict = {}
                text_spo_dict['text'] = test_set.text_list[batch_ndx]
                text_spo_dict['spo_list'] = []
                if len(output['selection_triplets'][0]) != 0:
                    text_spo_dict['spo_list'] = output['selection_triplets'][0]
                else:
...
if __name__ == "__main__":
    ...
    #preprocessing | train | evaluation | test
