...
from pytorch_pretrained.modeling import PreTrainedBertModel, BertModel
...
class HBT(PreTrainedBertModel):
    ...
    def forward(self,
        ...
        # subject_logits
        pred_sub_heads = torch.sigmoid(self.subject_head_layer(sequence_output))  # [batch_size, seq_len, 1]
        pred_sub_tails = torch.sigmoid(self.subject_tail_layer(sequence_output))
        ...
        if positions is not None:
            ...
            # obj_logits
            pred_obj_heads = torch.sigmoid(self.object_head_layer(tokens_feature))   # [batch_size, seq, rel_nums]
            pred_obj_tails = torch.sigmoid(self.object_tail_layer(tokens_feature))   # [batch_size, seq, rel_nums]
