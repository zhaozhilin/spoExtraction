...
class my_tokenizer():
    ...
    def _tokenize(self, text):
        ...
        for w in text:
            if w != ' ':
                text_out += w
        return self.tokenizer.tokenize(text_out)
...
def extract_items(bert, subject_model, object_model, tokenizer, text_in, id2rel, h_bar=0.5, t_bar=0.5):
    text = tokenizer._tokenize(text_in)
...
def metric(bert, subject_model, object_model, eval_data, id2rel, tokenizer, exact_match=True, output_path=None):
    if output_path:
