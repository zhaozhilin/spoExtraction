...
def load_data(data_path, tokenizer, rel2id, num_rels, maxlen=512):
...
def process_data(data, tokenizer, rel2id, num_rels, maxlen=512):
    ...
    for idx in tqdm(idxs):
        ...
        s2ro_map = {}
        # key:(object的起始位置，object的末尾)   value:[(object的起始位置, object的末尾, 位置关系索引),(),(),……]
        for triple in triples:
            triple = (tokenizer._tokenize(triple[0]), triple[1], tokenizer._tokenize(triple[2]))
        ...
        if s2ro_map:
            ...
            sub_heads = np.zeros(text_len)#初始化subheads
            sub_tails = np.zeros(text_len)
            for s in s2ro_map:
                sub_heads[s[0]] = 1#将实体位置的头位置标为1
                sub_tails[s[1]] = 1#将实体位置的尾位置标为1
            ...
            for ro in s2ro_map.get(tuple(sub_positions), []):
                obj_heads[ro[0]][ro[2]] = 1#某种关系的object的头位置
